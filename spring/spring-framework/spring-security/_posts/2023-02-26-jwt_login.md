...
public class SecurityConfig {
	...
	public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
		http
                  ...
                  .and()
                  .authorizeHttpRequests()  //HttpServletRequest 요청 URL에 따라 접근 권한 설정
                  .antMatchers("/api/v1/members/signup", "/api/v1/members/login", "/swagger-ui.html")  //요청 URL 경로 패턴 지정
                  .permitAll()  //모든 유저 접근 허용
                  .and()
                  .addFilterBefore(new JwtAuthenticationFilter(jwtTokenProvider), UsernamePasswordAuthenticationFilter.class);  
                  //구현한 JwtAuthenticationFilter 추가, UsernamePasswordAuthenticationFilter  
...
토큰에 담긴 정보에 맞는 authentication 객체를 반환해줌   
...
JwtTokenProvider가 준 Authentication 객체(인증 완료, 권한 부여된)를 SecurityContext에 저장    
...
public class JwtAuthenticationFilter extends GenericFilterBean {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		//JwtTokenProvider에서 토큰 유효성 검사
		if (StringUtils.hasText(token) && jwtTokenProvider.validateToken(token)) {
			//토큰이 유효할 경우 JwtTokenProvider에서 Authentication 객체 받음  
